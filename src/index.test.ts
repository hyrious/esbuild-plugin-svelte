...
const compile_file = async (
  ...
  assert.equal(js.map.sources, [basename(filename), ...Object.keys({ ...addedSources })])
...
ts('should work with `src`', async () => {
  ...
  const external = await readFile('./test/nested/script.ts', 'utf-8')
  await compile_file(
    filename,
    {
      onwarn(message) {
